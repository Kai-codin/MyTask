...
function Tasks() {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const res = await fetch('https://mytask.kaicoding.cc:3000/api/tasks', {
          method: 'GET',
          headers: {
            ...getAuthHeader(),
  ...
  const handleTaskSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Refresh tasks list
        const refreshed = await fetch('https://mytask.kaicoding.cc:3000/api/tasks', {
  ...
  const handleCompleteTask = async (taskId) => {
    try {
      const res = await fetch(`https://mytask.kaicoding.cc:3000/api/tasks/${taskId}/complete`, {
        method: 'PUT',
        headers: getAuthHeader(),
        credentials: 'include',
      });
  ...
  const handleUncompleteTask = async (taskId) => {
    try {
      const res = await fetch(`https://mytask.kaicoding.cc:3000/api/tasks/${taskId}/uncomplete`, {
        method: 'PUT',
        headers: getAuthHeader(),
        credentials: 'include',
      });
  ...
  const handleDeleteTask = async (taskId) => {
    ...
    try {
      const res = await fetch(`https://mytask.kaicoding.cc:3000/api/tasks/${taskId}`, {
        method: 'DELETE',
        headers: getAuthHeader(),
        credentials: 'include',
      });
  ...
  const handleUpdateTask = async (e) => {
    ...
    try {
      const res = await fetch(`https://mytask.kaicoding.cc:3000/api/tasks/${editingTask.id}`, {
        method: 'PUT',
        headers: {
          ...getAuthHeader(),
          'Content-Type': 'application/json'
